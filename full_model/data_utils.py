...
#补充：这里按照句子长度由大到小排序是因为模型rnn那里，nn.utils.rnn.pack_padded_sequence方法默认输入数据的句子长度是由大到小排序的。
    def __init__(self, data, batch_size, num_steps, cls_id, sep_id, pad_id):
    ...
    def sort_and_pad(self, data, batch_size, num_steps):
        ...
        for i in trange(self.num_batch):
            batch_data.append(self.pad_data(sorted_data[i*batch_size : (i+1)*batch_size],num_steps,self.cls_id,self.sep_id,self.pad_id))
        return batch_data

    @staticmethod
    def pad_data(data, length, cls_id, sep_id, pad_id):
    ...
    #strings是把data所有句子的字符串形成一个数组，lens是data所有句子的长度，chars是把strings中的字符串映射成id，segs是entity-type的id，subtypes是entity_subtype的id，targets是bio标签，adj是max_length*max_length的邻接矩阵，dep是max_length*max_length的依赖矩阵
        ...
        max_length = length
        for line in data:
            string, char, seg, subtype, target, dep_rel_features, dep_word_idx, pos, char_ori= line
            ...
            if string_len <= max_length:
                ...
                chars_ori.append(char_ori + padding)
                padding_bert = [pad_id] * (max_length - len(string))
                chars.append([cls_id]+char+[sep_id]+ padding_bert)
                masks_bert.append([1]*(len(string)+2)+[0]*(max_length - len(string)))
                masks_select.append([False]+[True]*len(string)+[False]+[True]*(max_length - len(string)))
            else:
                ...
                chars_ori.append(char_ori[0:max_length])
                chars.append([cls_id]+char[0:max_length]+[sep_id])
                masks_bert.append([1]*(max_length+2))
                masks_select.append([False]+[True]*max_length+[False])
            # Dep:
            ...
            adj.append(curr_adj)
            dep.append(curr_dep)
        return [strings, lens, chars, segs, subtypes, targets, adj, dep, poses, masks_bert, masks_select, chars_ori]
