...
class EDModel(nn.Module):
    ...
    def __init__(self, args, id_to_tag, device, pre_word_embeds):
        super(EDModel, self).__init__()
        self.m = args.m
        self.classes_num = len(id_to_tag)
        self.device = device
        self.gcn_model = EEGCN(device, args, pre_word_embeds)
        self.gcn_dim = args.gcn_dim
        self.classifier = nn.Linear(self.gcn_dim, self.classes_num)
        self.bert_classifier = BertClassifier(args, self.classes_num, device)
        self.bert_classifier.load_state_dict(torch.load("./models/test_finetune_bert_5_cla.pkl"),strict=False)
        #第二组实验调整freeze的层，代码如下
        unfreeze_layers=["bert_classifier"]
        for name ,param in self.bert_classifier.named_parameters():
    ...
    def forward(self, word_sequence, x_len, entity_type_sequence, adj, dep, pos, masks_bert, masks_select, char_ori):
        outputs, weight_adj = self.gcn_model(word_sequence, x_len, entity_type_sequence, adj, dep, pos, masks_bert, masks_select, char_ori)
        logits = self.classifier(outputs)
        logits_bert = self.bert_classifier(word_sequence, x_len, masks_bert, masks_select)
       
        final_logits = logits* self.m + logits_bert * (1.0-self.m)
        
        return final_logits, weight_adj


class EEGCN(nn.Module):
    def __init__(self, device, args, pre_word_embeds):
        super().__init__()
        self.word_embed_dim = args.word_embed_dim
        self.device = device
        self.in_dim = args.word_embed_dim + args.bio_embed_dim
        self.maxLen = args.num_steps
        self.bert_out_dim = args.bert_out_dim
        ...
        #self.adapt_fc = nn.Linear(self.bert_dim, args.bert_out_dim)
    ...
    def forward(self, word_sequence, x_len, entity_type_sequence, adj, edge, pos, masks_bert, masks_select, char_ori):
        ...
        #word_sequence = word_sequence[:, :BATCH_MAX_LEN+2].contiguous()
...
#         x_emb = word_emb
        ...
        if self.bio_embed_dim:
            ...
            word_emb_ori = torch.cat([word_emb_ori,entity_label_emb], dim=2)

        rnn_outputs = self.rnn_drop(self.encode_with_rnn(word_emb_ori, x_len, BATCH_SIZE))
