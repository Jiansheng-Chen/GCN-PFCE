...
def eval_model(id_to_char, id_to_tag, test_manager, device, model_name=None):
    ...
    for batch in test_manager.iter_batch():
        ...
        logits,_ = e_model(chars, _lens, subtypes, adj, dep, pos, masks_bert, masks_select, chars_ori)
...
def train(id_to_char, id_to_tag, train_manager, dev_manager, device):
    ...
    model.load_state_dict(torch.load("./models/eegcned-master-pos-1.7-elu-wd1e-4.pkl"),strict=False)
    
    #model = Model(args, id_to_tag, device).to(device)
    bert_params_id = list(map(id, model.bert_classifier.parameters()))
    rest_params = filter(lambda p: id(p) not in bert_params_id, model.parameters())
    params = [{"params": rest_params, "lr": args.lr},{"params": model.bert_classifier.parameters(),"lr": args.lr_bert}]
    optimizer = 0
...
#             print(name , param.shape)
    if args.optimizer == 'Adam':
        ...
        optimizer = torch.optim.SGD(params, weight_decay=args.weight_decay)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--m', default=0.75, type=float)
